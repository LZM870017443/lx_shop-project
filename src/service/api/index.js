...
export const getPhoneCode = (phone) =>
  ajax(LOCAL_BASE_URL + "/api/send_code", { phone });
// 2.2 手机验证码登录(POST)
...
export const pwdLogin = (user_name, user_pwd, captcha) =>
  ajax(
    LOCAL_BASE_URL + "/api/login_pwd",
    { user_name, user_pwd, captcha },
    "POST"
  );
// 2.4 自动登录
...
export const addGoodsToCart = (
  ...
  ajax(
    LOCAL_BASE_URL + "/api/cart/add",
    { user_id, goods_id, goods_name, goods_price, small_image },
    "POST"
  );

// 3.2 获取当前用户购物车中的商品
export const getGoodsCart = (user_id) =>
  ajax(LOCAL_BASE_URL + "/api/cart/search/" + user_id);

// 3.3 修改购物车商品数量
export const changeCartNum = (user_id, goods_id, type) =>
  ajax(LOCAL_BASE_URL + "/api/cart/num", { user_id, goods_id, type }, "POST");

// 3.4 删除当前用户购物车中所有的商品
export const clearAllCart = (user_id) =>
  ajax(LOCAL_BASE_URL + "/api/cart/clear/" + user_id);

// 3.5 单个商品的选中和取消选中
export const singerGoodsSelect = (user_id, goods_id) =>
  ajax(
    LOCAL_BASE_URL + "/api/cart/singer_select",
    { user_id, goods_id },
    "POST"
  );

// 3.6 所有商品的选中和取消选中
export const allGoodsSelect = (user_id, flag) =>
  ajax(LOCAL_BASE_URL + "/api/cart/all_select", { user_id, flag }, "POST");

// 3.7 查询所有已经被选中的商品
export const getAllSelectedGoods = (user_id) =>
  ajax(LOCAL_BASE_URL + "/api/cart/selected/" + user_id);

// 3.8 删除已经生成订单的商品
export const delAllSelectedGoods = (user_id) =>
...
export const getUserAddress = (user_id) =>
  ajax(LOCAL_BASE_URL + "/api/address/search/" + user_id);
...
export const addUserAddress = (
  ...
  ajax(
    LOCAL_BASE_URL + "/api/address/add",
    {
      user_id,
      address_name,
      address_phone,
...
export const changeUserAddress = (
  ...
  ajax(
    LOCAL_BASE_URL + "/api/address/edit",
    {
      address_id,
      user_id,
      address_name,
...
export const orderPaySuccess = (user_id, order_id) =>
  ajax(
    LOCAL_BASE_URL + "/api/order/change_status",
    { user_id, order_id },
    "POST"
  );
...
export const getOrder = (user_id, status) =>
  ajax(LOCAL_BASE_URL + "/api/order/get", { user_id, status }, "POST"); // pay will
...
// const PAY_URL = '/pay';
// 6.1 获取支付的URL
export const getWXCode = (outTradeNo, totalFee) =>
  ajax(PAY_URL + "/createNative.do", { outTradeNo, totalFee });
// 6.2 查询是否支付成功
export const queryPayStatus = (out_trade_no) =>
  ajax(PAY_URL + "/queryPayStatus.do", { out_trade_no });
